...
public class DepartmentDaoImp extends BaseDao implements DepartmentDao {
	...
	public List findDepartmentByParentId(String parentId) throws DataAccessException {
		StringBuffer sql = new StringBuffer("select t1.ID,t1.NAME,t1.CODE,t1.SEQ,count(t2.id) as count");
		sql.append(" from HW_DEPARTMENT t1 left join HW_DEPARTMENT t2 on t1.ID = t2.PARENT_ID");
