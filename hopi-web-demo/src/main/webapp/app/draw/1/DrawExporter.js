Ext
		.define(
				'Hopi.draw.DrawExporter',
				{
					generate : function(s) {
						var surface=s, len=surface.length, width= surface.width, 
						...
						spriteProcessor = {
							...
							text : function(sprite) {
								var attr = sprite.attr, fontRegex = /(-?\d*\.?\d*){1}(em|ex|px|in|cm|mm|pt|pc|%)\s('*.*'*)/, match = fontRegex
										.exec(attr.font), size = (match && match[1])
										|| "12", family = (match && match[3]) || 'Arial', text = attr.text, factor = (Ext.isFF3_0 || Ext.isFF3_5) ? 2
										: 4, tspanString = '', props;
								...
								return '<text ' + props + '>' + tspanString
										+ '</text>';
							},
							rect : function(sprite) {
								var attr = sprite.attr, props = toPropertyString( {
									x : attr.x,
									y : attr.y,
									rx : attr.rx,
									ry : attr.ry,
							...
							var svg = '<svg width="'
									...
									+ 'px" height="'
									+ height
									+ 'px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1">', defs = '', item, itemsLen, items, gradient, getSvgString, colorstops, stop, coll, keys, colls, k, kLen, key, collI, i, o, stopsLen;

							items = surface.items.items;
							itemsLen = items.length;
							getSvgString = function(node) {
								var childs = node.childNodes, childLength = childs.length, i = 0, attrLength, o, svgString = '', child, attr, tagName, attrItem;
							...
							if (surface.getDefs) {
								...
								for (; k < kLen; k++) {
									...
									gradient = surface.gradientsColl
											.getByKey(key);
