Ext.define('Ado.ds.DsTablePanel', {
...
//							this.propertyStore.load();
	...
	addTc:function(){		
		...
		this.tcForm.getForm().findField('DB_ID').setValue(
				this.dbCombo.getValue());	
		this.tcForm.getForm().findField('PARENT_ID').setValue(
				this.selectTreeId);		
	},
	saveTc:function(){
	...
	initComponent : function() {
		this.tableBaseUrl = 'dsTable.do?method=';		
		this.databaseBaseUrl = 'dsDataBase.do?method=';
		this.propertyBaseUrl = 'dsProperty.do?method=';		
...
//		this.dbComboStore.load();
		
		this.treeStore =Ext.create('Ext.data.TreeStore', {
			proxy : {
				type : 'ajax',
				url : 'dsTable.do?method=tableTree'
			},
			root : {
				text : '根节点',
				draggable : false,
...
//						console.log(node.data.text);																
		...
		this.propertyStore = Ext.create('Ext.data.Store', {
			idProperty : 'id',
			fields : [ 'ID', 'NAME', 'CODE', 'SEQ', 'DATA_TYPE','DATA_TYPE_NAME',
					 'DATA_LENGTH', 'IS_PK', 'DESCRIPTION' ],
			proxy : {
				type : 'ajax',
